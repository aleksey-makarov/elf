...
getElf = do
    ...
    e_version2  <- getE ei_data
    when (e_version2 /= (1 :: Word32)) $ error "verification failed: version2"


    let 
        get64 = case ei_class of
            ELFCLASS32 -> fromW32 <$> getE ei_data
            ELFCLASS64 ->             getE ei_data

    e_entry     <- get64
    e_phoff     <- get64
    e_shoff     <- get64

    e_flags     <- getE ei_data

    (e_ehsize    :: Word16) <- getE ei_data

    (e_phentsize :: Word16) <- getE ei_data
    (e_phnum     :: Word16) <- getE ei_data
    (e_shentsize :: Word16) <- getE ei_data
    (e_shnum     :: Word16) <- getE ei_data

    e_shstrndx  <- getE ei_data

    hSize <- bytesRead
    when (hSize /= fromIntegral e_ehsize) $ error "incorrect size of elf header"

    e_xx <- case ei_class of
        ELFCLASS32 -> Elf32 <*> getTable
                            <$> getTable
        ELFCLASS64 -> Elf64 <*> getTable
                            <$> getTable
...
getElfSection64 d = ElfSection64 <$> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
...
getElfSection32 d = ElfSection32 <$> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
...
getE :: (Binary (Le a), Binary (Be a)) => ElfData -> Get a
getE ELFDATA2LSB = fromLe <$> get
getE ELFDATA2MSB = fromBe <$> get
...
getElfSegment64 d = ElfSegment64 <$> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
...
getElfSegment32 d = ElfSegment32 <$> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
                                 <*> getE d
